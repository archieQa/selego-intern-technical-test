...
const FileInput = ({ value, onChange, name, folder, disabled = false }) => {
  ...
  if (!value) {
    ...
    preview = <img className="object-contain w-20 h-20" src={value} alt="user.png" />
  }
  ...
  return (
    <div className="relative w-28 h-28 overflow-visible">
      ...
      {loading ? (
        ...
        <button type="button" className="bg-gray-200 w-full h-full text-xs flex justify-center items-center px-2" onClick={() => inputRef.current.click()} disabled={disabled}>
        ...
        <Popover className="relative">
          ...
          >
            <Popover.Panel className="absolute left-1/2 z-10 flex w-screen max-w-min -translate-x-1/2 px-4">
              <div className="flex flex-col w-32 shrink rounded-xl overflow-hidden bg-white text-sm leading-6 text-gray-900 shadow-lg ring-1 ring-gray-900/5 divide-y divider-gray-200">
